...
type Snapshot struct {
	config   *params.BposConfig // Consensus engine parameters to fine tune behavior
...
// the genesis block.
func newSnapshot(config *params.BposConfig, sigcache *lru.ARCCache, number uint64, hash common.Hash, validators []common.Address) *Snapshot {
	snap := &Snapshot{
		config:     config,
		sigcache:   sigcache,
		Number:     number,
